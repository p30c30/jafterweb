// ===================================================================
// ==        SCRIPT.JS - VERSIÓN FINAL Y COMPLETA (v36)           ==
// ===================================================================

console.log('✅ script.js v36 CARGADO');
...
function iniciar() {
  ...
  setTimeout(() => {
    ...
    if (container) { cargarDatos(container); } else { setTimeout(iniciar, 1000); }
  }, 600);
...
function configurarInteraccionCarrusel() {
  ...
  function onStart(e) {
    isDraggingLocal = true; dx = 0;
    startX = (e.touches ? e.touches[0].clientX : e.clientX);
    inner.style.transition = 'none'; stopCarouselAutoplay();
  }
  function onMove(e) {
...
function mostrarModal(imageUrl, title, fotoIndex, opts = { push: true, source: null }) {
  ...
  const onImageLoad = function () {
    ...
    configurarEventosModal();     // engancha la lógica de UI/teclado/gestos
    precacheAround(currentFotoIndex);

    // Mostrar toda la UI 3s al abrir
  ...
  function configurarEventosModal() {
    ...
    const modalImgLocal = modal.querySelector('#modal-img');

    // === Auto-ocultar UI con temporizador y hotspots estables ===
    ...
    // Exponer hook global para cuando cambia la foto
    window.triggerUiAfterPhotoChange = () => showAllAndArmTimer();

    // Mover el ratón en cualquier parte => mostrar Cerrar y rearmar
    modal.addEventListener('mousemove', () => { showCloseAndArmTimer(); });

    // Hotspots muestran su UI (evitamos ocultar en mouseleave para no "flicker")
    ...
    hotspotBottom?.addEventListener('mouseenter', () => { show(infoPanel); resetTimer(); });
    ...
    // Tap/click en imagen => toggle zoom
    let tapStartX = 0, tapStartY = 0, tapStartT = 0;
    modalImgLocal.addEventListener('touchstart', (e) => {
    ...
    modalImgLocal.addEventListener('touchend', (e) => {
      if (ignoreNextClick) { ignoreNextClick = false; return; }
    ...
    modalImgLocal.addEventListener('click', function (event) {
      if (ignoreNextClick) { ignoreNextClick = false; event.stopPropagation(); return; }
      doClickToggle();
      event.stopPropagation();
    });
    modalImgLocal.addEventListener('dblclick', (e) => e.preventDefault());
    ...
    // Drag/Pan y pinch
    modalImgLocal.addEventListener('mousedown', startDrag);
    modalImgLocal.addEventListener('touchstart', onTouchStartImg, { passive: false });
...
function precacheAround(index) {
  ...
  [ (index + 1) % n, (index - 1 + n) % n ].forEach(i => {
    const im = new Image(); im.src = list[i].url;
  });
}
...
function stopDrag() {
  ...
  if (currentImage && currentScale > 1) {
    currentImage.style.cursor = 'move'; currentImage.classList.remove('grabbing'); startInertia();
  }
...
function resetZoom() {
  ...
  if (inertiaId) { cancelAnimationFrame(inertiaId); inertiaId = null; }
  if (currentImage) {
    currentImage.style.transition = ''; currentImage.style.transform = 'scale(1) translate3d(0px, 0px, 0)';
    currentImage.classList.remove('zoomed', 'grabbing'); currentImage.style.cursor = 'default';
  }
...
function navegarFoto(direccion) {
  ...
  im.onload = function () {
    ...
    // Mostrar toda la UI 3s tras cambiar de foto
    if (typeof window.triggerUiAfterPhotoChange === 'function') {
      window.triggerUiAfterPhotoChange();
    }
  };
...
function attachSwipeToModal(modal) {
  ...
  function onMove(e) {
    ...
    if (!blockVertical && Math.abs(dy) > Math.abs(dx) && Math.abs(dy) > 10) {
      blockVertical = true; modal.classList.remove('is-gesturing');
    }
  }
  function onEnd(e) {
...
function attachBottomSheet(modal) {
  ...
  function stopScrollBounce(el) {
    ...
    let tsY = 0;
    el.addEventListener('touchstart', (e) => { if (e.touches.length !== 1) return; tsY = e.touches[0].clientY; }, { passive: true });
  ...
  handle.addEventListener('touchstart', (e) => {
    const t = e.touches[0]; dragging = true; dragStartY = t.clientY;
  ...
  handle.addEventListener('touchmove', (e) => {
    if (!dragging) return;
    const t = e.touches[0]; const dy = t.clientY - dragStartY;
...
function toggleFullscreen() {
  ...
  const restorePanel = () => {
    ...
    modal.style.removeProperty('--info-height'); aplicarZoom(true);
  };
